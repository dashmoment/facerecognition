...
class App(object):
    ...
    def run_img(self):
        ...
        frame = cv2.imread('test/s4/9.pgm')
        #img = cv2.resize(frame, (frame.shape[1]/2, frame.shape[0]/2), interpolation = cv2.INTER_CUBIC)
        frame = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
        img = cv2.resize(frame, self.model.image_size, interpolation = cv2.INTER_CUBIC)
    
        prediction = self.model.predict(img)[0]
        print "Prediction result format {}".format(self.model.predict(img))
        
        if prediction == 0:
            print "Who is this guy?"
        else:
            output =  "No.{}".format(self.model.subject_names[prediction])
            
        
        draw_str(frame, (20,20), output)
...
if __name__ == '__main__':
    ...
    App(model=model,
        ...
        cascade_filename=options.cascade_filename).run_img()
